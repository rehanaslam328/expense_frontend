...
const { NAME, NEW_SEAT_PREFERENCE, ENTER, CREATE, CANCEL } = Labels;
...
const Modalx = ({ visible, loading, toggle, onSubmit, current }: ModalProps) => {
  ...
  return (
    ...
    >
      <Form name={NEW_SEAT_PREFERENCE} initialValues={initialState} form={form} onFinish={onSubmit}>
        <InputField
          size="middle"
          name="name"
          label={NAME}
...
export const SeatForm = ({ onSubmit, bool, loading, toggle, current }: FormProps) => (
  <>
    <Modalx
      visible={bool}
      loading={loading}
